    ...
    ({
        ...
        const onSetCurrentDropdown = (e, id) => {
            ...
            if (multiple) {
                ...
                setDropdownCurrent(
                    dropdownCurrent === e.currentTarget.textContent
                        ? false
        ...
        const typesList = typeList.map((item) => {
            return (
                <li
                    key={item.id}
                    onClick={(e) => onSetCurrentDropdown(e, item.id)}
            ...
            <div ref={ref} className={`dropdown ${dropdownToggle ? 'active' : ''}`} tabIndex="1">
                <div className="dropdown__current" onClick={onDropdownActive}>
                    <div className="dropdown__current-item">
                        {!dropdownCurrent ? type : dropdownCurrent}
                    </div>
                ...
                <ul className="dropdown__menu">
                    <li
                        ...
                        className={`dropdown__menu-item ${
                            multiple ? (state.length === 0 ? 'active' : '') : !state ? 'active' : ''
                        }`}>
                        {type}
                    </li>
                    {typesList.length ? (
                        ...
                        <div className="dropdown__menu-item">Подкатегории отсутствуют</div>
                    )}
                </ul>
            </div>
        );
