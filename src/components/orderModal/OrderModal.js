    ...
    const placeOrder = () => {
        if (!loading) {
            ...
            if (
                ...
                moment(date).format('LL') === moment(startDate).format('LL') &&
                startTime.slice(0, 2) < date.getHours() + 3
            ) {
                setInputError(`Доставка не раньше ${date.getHours() + 3} часов`);
            } else {
                setLoading(true);
                orderVerification().then((data) => {
        ...
        <>
            ...
            <div className="order">
                ...
                <div className="order__price">
                    Итоговая стоимость: <span>{`${items.totalPrice} ₽`}</span>
                </div>
                <div
                ...
                <motion.button
                    ...
                    {loading ? <span className="loader"></span> : 'Оформить'}
                </motion.button>
            </div>
        </>
    );
