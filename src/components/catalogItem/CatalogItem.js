...
const CatalogItem = observer(({item}) => {
    ...
    const calcPlus = () => {
        if (flag !== -1) {
            if (items.cart[flag][1] > 0) {
                ...
                addToCart(item.id, items.cart[flag][1]+1, item.price, items)
            }
        } else {
            if (count > 0) {
    ...
    return (
        <>
        {changeModal && <ChangeModal changeModal={showAnimation} setChangeModal={setChangeModal} showAnimation={showAnimation} setShowAnimation={setShowAnimation} item={item} />}
        <motion.div
            ref={rootRef}
        ...
        >
            {user.isAuth && 
                ...
                <div className="market__item-icon" onClick={() => {setChangeModal(true); setShowAnimation(true)}}>
                ...
                <div className="market__item-bottom">
                    ...
                    <div className="market__item-group">
                        ...
                        <motion.div
                            ...
                            whileTap={{ scale: 0.9 }}
                            onClick={() => addToCart(item.id, count, item.price, items)}
