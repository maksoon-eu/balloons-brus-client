...
const Catalog = observer(({updateList, setUpdateList}) => {
    ...
    useEffect(() => {
        if (itemList.length === 0 || updateList) {
    ...
    }, [items.selectedType, items.selectedSubType, items.selectedPrice, items.itemsSort, updateList])
    ...
    return (
        <>
            ...
            <div className="sort" ref={refSort}>
                ...
                <SortModal setUpdateList={setUpdateList} sortModal={sortModal} setSortModal={setSortModal} refSort={refSort}/>
            </div>
