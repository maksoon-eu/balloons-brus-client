...
const Catalog = observer(() => {
    ...
    useEffect(() => {
        if ((items.items.length === 0 && items.itemsLoading) || items.updateList) {
            ...
            const fetchDataPromise = fetchItems(
                items.selectedType,
                items.selectedSubType,
                items.selectedPrice,
                1,
                12 * items.page,
                items.itemsSort
        ...
        <>
                ...
                <AnimatePresence mode="wait">
                    <motion.div
                        ...
                        {items.itemsLoading === 'loading' ? (
                            skeletonList
                        ) : items.itemsLoading !== 'loading' && itemList.length === 0 ? (
            ...
            {items.totalCount % 12 === 0 && items.totalCount !== 0 && (
                <motion.div
                    ...
                    whileTap={{ scale: 0.9 }}
                    onClick={loadMore}>
                    {items.itemsLoading === 'loading' || items.itemsLoading === 'updateLoading' ? (
